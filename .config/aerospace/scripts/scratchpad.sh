...
# Find scratchpad window (any terminal window with title containing "scratchpad")
SCRATCH_WID=$(aerospace list-windows --all --format "%{window-id}|%{workspace}|%{window-title}" 2>/dev/null \
  | grep -i "scratchpad" \
  | head -1)

if [ -z "$SCRATCH_WID" ]; then
  # No scratchpad exists — create one using default terminal
  osascript -e '
    tell application "Terminal"
      do script "printf \"\\033]0;scratchpad\\007\"; clear"
      activate
    end tell' 2>/dev/null
  ...
  # Find the new Terminal window and float it
